...
namespace Vars
{
	...
	NAMESPACE_BEGIN(Aimbot)
		SUBNAMESPACE_BEGIN(General)
			...
			CVar(HitscanPeek, 1)
			CVar(PeekDTOnly, true)
			CVar(NoSpreadOffset, 0.f)
			CVar(NoSpreadAverage, 5)
			CVar(NoSpreadInterval, 0.1f)
			CVar(NoSpreadBackupInterval, 2.f)
			CVarEnum(AimHoldsFire, 1, False, MinigunOnly, Always)
		...
		SUBNAMESPACE_BEGIN(Projectile)
			...
			CVar(GroundSamples, 55)
			CVar(GroundStraightFuzzyValue, 50.f)
			CVar(GroundLowMinimumSamples, 8)
			CVar(GroundHighMinimumSamples, 16)
			CVar(GroundLowMinimumDistance, 0.f)
			CVar(GroundHighMinimumDistance, 2500.f)
		    CVar(GroundMaxChanges, 0)
		    CVar(GroundMaxChangeTime, 0)

			CVar(AirSamples, 66)
			CVar(AirStraightFuzzyValue, 0.f)
			CVar(AirLowMinimumSamples, 3)
			CVar(AirHighMinimumSamples, 3)
			CVar(AirLowMinimumDistance, 100000.f)
			CVar(AirHighMinimumDistance, 100000.f)
		    CVar(AirMaxChanges, 1)
			CVar(AirMaxChangeTime, 10)

			CVar(VelocityAverageCount, 5)
			CVar(VerticalShift, 5.f)
			CVar(LatencyOffset, 0.f)
			CVar(HullIncrease, 0.f)
			CVar(DragOverride, 0.f)
			CVar(TimeOverride, 0.f)
			CVar(HuntsmanLerp, 50.f)
			CVar(HuntsmanLerpLow, 100.f)
			CVar(HuntsmanAdd, 0.f)
			CVar(HuntsmanAddLow, 0.f)
			CVar(HuntsmanClamp, 5.f)
			CVar(SplashGrates, true)
			CVarEnum(RocketSplashMode, 1, SpecialHeavy, SpecialLight, Regular)
			CVar(SplashPoints, 400)
			CVar(SplashCountDirect, 100)
			CVar(SplashCountArc, 100)
			CVar(SplashTraceInterval, 10)
			CVar(DeltaCount, 5)
			CVarEnum(DeltaMode, 0, Average, Max)
			CVar(StrafeDelta, false)
